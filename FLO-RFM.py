...
plt.boxplot(df["order_num_total_ever_online"],vert=False)
plt.title("Boxplot")
plt.show()

plt.violinplot(df["order_num_total_ever_online"], vert=True)
plt.title("Violin Plot")
plt.show()

plt.scatter(range(len(df)),df["order_num_total_ever_online"])
...
plt.pie(df["order_channel"].value_counts(), labels=df["order_channel"].unique(), autopct='%1.1f%%')
plt.title("Pie Chart")
plt.axis('equal')
plt.show()

sns.countplot(x="order_channel", hue="order_num_total", data=df,)
plt.title("Stacked Bar Plot")
plt.show()

crosstab = pd.crosstab(df["order_channel"], df["interested_in_categories_12"])
...
ax[0].set_xlabel("Müşteri Sayısı")
ax[0].set_ylabel("Sipariş Kanalı")
ax[0].set_title("Alışveriş Kanalına Göre Müşteri Dağılımı")

sns.kdeplot(df,
             x="overall_order_num",
             ax=ax[1],
             hue="order_channel")
ax[1].get_legend().set_title("Sipariş Kanalı")
ax[1].set_xlabel("Satın Alma Miktarı")
ax[1].set_ylabel("Müşteri Sayısı")
ax[1].set_title("Alışveriş Kanalına Göre Satın Alma Miktarı")
ax[1].set_xlim([0, 75])

sns.kdeplot(df,
             x="overall_customer_value",
             ax=ax[2],
             hue="order_channel")
ax[2].get_legend().set_title("Sipariş Kanalı")
ax[2].set_xlabel("Harcama Miktarı")
ax[2].set_ylabel("Müşteri Sayısı")
ax[2].set_title("Alışveriş Kanalına Göre Harcama Miktarı")
